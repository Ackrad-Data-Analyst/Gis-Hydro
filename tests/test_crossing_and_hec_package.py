...
class _Conversion:
    def FeaturesToJSON(self, source, output, *args): Path(output).write_text('{"type":"FeatureCollection"}')


class _ArcPy:
    def __init__(self):
        self.outputs = set(); self.management = _Management(self); self.analysis = _Analysis(self); self.conversion = _Conversion()
    def Exists(self, value): return value in {"roads", "drainage", "bridges", "culverts", "terrain", "streams"} or value in self.outputs
